...
.#! conf/vim-color
    ...
    option mirror.copies        =   frodo gollum
.end
...
.end
    ...
    The special value "true" to say that all copies are trusted is now "all":
.#! conf/vim-color
...
.end
  ...
  * The command to resync mirrors is 'mirror', run just like any other
    ...
    get help.  It cannot be run from a copy to ask a master to push (unlike
