...
`config gitolite-options.foo = 1`, so everything in the [git-config](git-config.md) page
also applies here (especially the bit about [overriding config
values](git-config.md#overriding-config-values).
...
Here are some examples, although this list is not exhaustive:
  ...
  * Options related to [mirroring](mirroring.md) -- tell gitolite who is the master server,
...
in a group (which you might guess from reading the [git-config](git-config.md) page):
